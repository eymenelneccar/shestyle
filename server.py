...
SETTINGS_ASSETS_PATH = os.path.join(ROOT_DIR, 'assets', 'settings.json')
    ...
    def do_POST(self):
        if self.path == '/api/save-menu':
            ...
            try:
                ...
                return
            try:
                os.makedirs(os.path.dirname(SETTINGS_ASSETS_PATH), exist_ok=True)
                with open(SETTINGS_ASSETS_PATH, 'w', encoding='utf-8') as f:
                    json.dump(data, f, ensure_ascii=False, indent=2)
                self.send_response(200)
                self.send_header('Content-Type', 'application/json; charset=utf-8')
                self.end_headers()
